...
public class LoginUI {
    ...
    public JPanel buildLoginPanel() {
        ...
        /* 
        - creates the username and password labels and fields
        - creates Login and Register buttons and attaches action listeners * rough Draft*
        - (loginBtn -> onLogin, registerBtn -> onRegister) * Actions explained, in sequence*
        - the buttons panel is added to the center area so user can submit * make it easier on users/drivers*
        - * use standard Swing components, to make it lightweight and compatible, unless something else is needed*
    ...
    registerBtn.addActionListener(this::onRegister);
        ...
        // wrap the centerPanel in a translucent card so controls are readable * will change in future*
   ...
   -  if admin, prompts for hash code
   -  if not admin, verifies credentials and user type
   -  updates messageLabel with success or failure messages
   - onRegister:
   -  prompts user to select user type (Customer/Driver)
   -  collects username, password, and additional info
   -  registers user in the database
   - for drivers, collects vehicle info and registers in driver database
   - creates and switches to DriverScreen if driver
   -  shows success message for customers
   - The logic used when the user clicks the "Login" or "Register" button on the "Login" page.
   - removed the CSV file, since database project
     */ 
   ...
   public void onLogin(ActionEvent e) {
            ...
            if (ok) {
                ...
                if ("DRIVER".equals(userType)) {
                    ...
                    try {
                        ...
                        // Scene already exists, that's fine * just ignore and switch to it*
                    }
        ...
        } catch (SQLException ex) { // database error
            JOptionPane.showMessageDialog(null, "Database error: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private String showUserTypeDialog() {
        ...
        // Create buttons for Customer and Driver
        JPanel buttonPanel = new JPanel(new GridLayout(2, 1, 10, 10));
        buttonPanel.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));
        
        JButton customerBtn = new JButton("Customer");
        JButton driverBtn = new JButton("Driver");
        // To capture the result from button clicks
        final String[] result = {null};
       // Action listeners to set result and close dialog 
...
// Add buttons to panel and show dialog
    ...
    public void onRegister(ActionEvent e) {
        ...
        if (user == null) return;
        user = user.trim();
        if (user.isEmpty()) {   // validate non-empty
            JOptionPane.showMessageDialog(null, "Username cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Get password, if empty, show error and return
        ...
        // Get additional information, as in full name, email, phone
        ...
        // All inputs gathered, proceed to register
        String hash = FoodDeliveryLoginUI.sha256Hex(pass);
        try { // try to register user
            ...
            // Insert the new user record into the SQLite database. *user registration*
            parent.userDb.register(user, hash, userType, fullName.trim(), email.trim(), phone.trim());

            // If registering a driver, collect additional driver information and register in driver database, *As in vehicle info, license, area, thats if its necessary honestly*
            if ("DRIVER".equals(userType)) {
                // Get vehicle information
                String vehicleType = JOptionPane.showInputDialog(null,  // * haven't utilized yet*
                    "Enter your vehicle type (e.g., Car, Motorcycle, Bicycle):", 
                    "Driver Registration", //*Might remove */
                    JOptionPane.QUESTION_MESSAGE);
                if (vehicleType == null || vehicleType.trim().isEmpty()) { // * haven't utilized yet*
                ...
                String licenseNumber = JOptionPane.showInputDialog(null, 
                    "Enter your driver's license number:", // Might remove, not ultized yet*
                    "Driver Registration", 
                    JOptionPane.QUESTION_MESSAGE);
                ...
                String serviceArea = JOptionPane.showInputDialog(null, 
                    "Enter your preferred service area (ZIP code):", // not utilized yet* but will use this i think
                    "Driver Registration", // not needed place holder
                    JOptionPane.QUESTION_MESSAGE);
